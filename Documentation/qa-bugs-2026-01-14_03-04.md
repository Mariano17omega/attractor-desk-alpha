...
1) Web search flow forces artifact paths (missing reply/theme routes)
...
- Suggested fix: Either wire artifact-generation paths through `generateFollowup` -> `reflect` or remove these nodes until they are ready.
